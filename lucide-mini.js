...
(function (global) {
  ...
  function createElement(tag, attributes, children) {
    ...
    (children || []).forEach(([childTag, childAttributes, childChildren]) => {
      element.appendChild(createElement(childTag, childAttributes, childChildren));
    });
    return element;
  }
  ...
  function createIcons({ attrs = {}, root = document } = {}) {
    ...
    scope.querySelectorAll("[data-lucide]:not(svg)").forEach((placeholder) => {
      ...
      const svg = createElement("svg", svgAttributes, children);
