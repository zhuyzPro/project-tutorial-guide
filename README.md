# Zhuyz Pro-猪油仔 · 项目教程导航

一个用于发布和管理项目使用教程的轻量网站。前台按生活场景展示项目，读者可打开 Markdown 图文正文查看完整教程；后台可以管理分类、教程、封面和排序，并查看访问与点击统计。

## 已包含功能

- 首页分类：生活权益、外卖奶茶、打车出行、网购入会、红包现金、游戏相关、其他项目。
- 项目教程：按“卡片前方（未点击）/卡片后方（点击后）”分组维护标题、卡片简介、卡片说明、详情介绍、正文 Markdown、封面图、小提示与项目入口；启用状态在列表行左侧胶囊按钮直接切换。
- 后台管理：新增、编辑、删除、开关、移动和排序分类及项目。
- 数据中心：数据概览、访问趋势、访问来源、项目点击四个数据入口，统一支持今天、近 7 天、近 30 天、近 90 天和自定义日期筛选，并可按小时/按天查看趋势。
- 访问来源：按完整访客 IP 汇总访问次数，展示首次访问与最近访问时间；仅管理员登录后可见。
...
- 卡片后方：详情介绍、正文 Markdown（可直接粘贴任意长度的教程内容）、封面图 URL、小提示、入口 URL（可选）。
